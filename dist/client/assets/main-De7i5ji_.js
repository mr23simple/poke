import"./loadHeader-D6-0TxLk.js";/* empty css                     */document.addEventListener("DOMContentLoaded",async()=>{const p=document.getElementById("loading-overlay"),k=document.getElementById("rankings-grid"),l=document.getElementById("modal-backdrop"),u=document.getElementById("modal-content"),c={1:.094,1.5:.1351374318,2:.16639787,2.5:.192650919,3:.21573247,3.5:.2365726613,4:.25572005,4.5:.2735303812,5:.29024988,5.5:.3060573775,6:.3210876,6.5:.3354450362,7:.34921268,7.5:.3624577511,8:.3752356,8.5:.387592416,9:.39956728,9.5:.4111935514,10:.4225,10.5:.4329264091,11:.44310755,11.5:.4530599591,12:.4627984,12.5:.472336093,13:.48168495,13.5:.4908558003,14:.49985844,14.5:.508701765,15:.51739395,15.5:.5259425113,16:.5343543,16.5:.5426357375,17:.5507927,17.5:.5588305862,18:.5667545,18.5:.5745691333,19:.5822789,19.5:.5898879072,20:.5974,20.5:.6048236651,21:.6121573,21.5:.6194041216,22:.6265671,22.5:.6336491432,23:.64065295,23.5:.6475809666,24:.65443563,24.5:.6612192524,25:.667934,25.5:.6745818959,26:.6811649,26.5:.6876849038,27:.69414365,27.5:.70054287,28:.7068842,28.5:.7131691091,29:.7193991,29.5:.7255756136,30:.7317,30.5:.7377694897,31:.7437366,31.5:.749609799,32:.7553897,32.5:.761080073,33:.7666845,33.5:.772199568,34:.7776309,34.5:.782983635,35:.7882599,35.5:.793460966,36:.7985881,36.5:.803645071,37:.8086304,37.5:.813544863,38:.8183946,38.5:.823179625,39:.7846369743347168,39.5:.7874736085132754,40:.7903000116348267,40.5:.792803968023538,41:.7953000068664551,41.5:.7978038983716224,42:.8003000020980835,42.5:.8028038718775964,43:.8052999973297119,43.5:.8078038508470536,44:.8102999925613403,44.5:.812803835179168,45:.8152999877929688,45.5:.8178037928037928,46:.8203000020980835,46.5:.822803776019539,47:.82530000925064,47.5:.827803755931569,48:.830300023555755,48.5:.832803729034748,49:.835300018787384,49.5:.837803755931569,50:.840300023555755,50.5:.842803729034748,51:.845300018787384,51.5:.847803702398935,52:.850300014019012,52.5:.852803676019539,53:.85530000925064,53.5:.857803649892077,54:.860300004482269,54.5:.862803624012168,55:.865299999713897};function $(e){let t=null,s=1/0;for(const i in c){const m=Math.abs(c[i]-e);m<s&&(s=m,t=i)}return t}function y(e){const t=(s,i)=>{if(!i||!i.value||i.value<=1)return"";const m=i.text?`(${i.text})`:"";return`<li><span>${s} ${m}</span><span>1 in ${Math.round(i.value).toLocaleString()}</span></li>`};u.innerHTML=`
        ...
        `,l.classList.remove("hidden"),document.body.classList.add("modal-open"),document.getElementById("modal-close-btn").onclick=()=>{l.classList.add("hidden"),document.body.classList.remove("modal-open")}}function g(e){const t=[];let s=!1,i=!1,m=!1;return e.isShiny&&e.isLucky&&e.isPerfect?(t.push('<span class="badge shlundo-badge">Shlundo</span>'),s=i=m=!0):e.isLucky&&e.isPerfect?(t.push('<span class="badge lundo-badge">Lundo</span>'),s=m=!0):e.isShiny&&e.isPerfect?(t.push('<span class="badge shundo-badge">Shundo</span>'),s=i=!0):e.isShiny&&e.isZeroIv&&(t.push('<span class="badge shnundo-badge">Shnundo</span>'),s=i=!0),!i&&e.isShiny&&t.push('<span class="badge shiny-badge">Shiny</span>'),!m&&e.isLucky?t.push('<span class="badge lucky-badge">Lucky</span>'):e.isTraded&&!e.isLucky&&t.push('<span class="badge traded-badge">Traded</span>'),s||(e.isPerfect?t.push('<span class="badge perfect-badge">Hundo</span>'):e.isZeroIv&&t.push('<span class="badge zero-iv-badge">Nundo</span>')),e.pokemonDisplay?.alignment===1&&t.push('<span class="badge shadow-badge">Shadow</span>'),e.pokemonDisplay?.alignment===2&&t.push('<span class="badge purified-badge">Purified</span>'),e.isLegendary&&t.push('<span class="badge legendary-badge">Legendary</span>'),e.isMythical&&t.push('<span class="badge mythical-badge">Mythical</span>'),t.join(" ")}function b(e){const t=e.iv.attack,s=e.iv.defense,i=e.iv.stamina,m=e.cpm,x=$(m);u.innerHTML=`
            <button id="modal-close-btn">&times;</button>
            <div class="pokemon-modal-header">
                <img src="${e.sprite}" alt="${e.name}">
                <div class="pokemon-info">
        ...
        `,l.classList.remove("hidden"),document.body.classList.add("modal-open"),document.getElementById("modal-close-btn").onclick=()=>{l.classList.add("hidden"),document.body.classList.remove("modal-open")},setTimeout(()=>{const M=document.getElementById("attack-bar");M.style.width=`${t/15*100}%`,M.style.backgroundColor=t===15?"#da7a79":"#f79513";const E=document.getElementById("defense-bar");E.style.width=`${s/15*100}%`,E.style.backgroundColor=s===15?"#da7a79":"#f79513";const h=document.getElementById("stamina-bar");h.style.width=`${i/15*100}%`,h.style.backgroundColor=i===15?"#da7a79":"#f79513"},100)}function v(e){return!e||e.length===0?"":`--pokemon-bg: ${e.length===1?e[0]:`linear-gradient(135deg, ${e[0]} 30%, ${e[1]} 70%)`};`}const d=localStorage.getItem("liteMode")==="enabled";async function n(e){u.innerHTML=`
            <button id="modal-close-btn">&times;</button>
        ...
        `,l.classList.remove("hidden"),document.body.classList.add("modal-open"),document.getElementById("modal-close-btn").onclick=()=>{l.classList.add("hidden"),document.body.classList.remove("modal-open")};try{const t=await fetch(`/api/player-detail/${e}`);if(!t.ok)throw new Error("Could not fetch player details.");const s=await t.json();u.innerHTML=`
                ...
                <div id="modal-pokemon-container">
                    ${s.highlights.map(i=>`<div class="${`pokemon-card${i.typeColors.length>0?" colored":""}${d||!i.sprite?" no-image":""}`}" style="${v(i.typeColors)}">
                                    <img src="${i.sprite}" alt="${i.name}" loading="lazy">
                                    <p class="pokemon-name">
                                        ${d?`<span class="lite-name-span" style="${v(i.typeColors)}">${i.name}</span>`:i.name}
                                    </p>
            ...
            <tr class="clickable-row" data-player-id="${a.publicId}">
                ...
                <td>
                    ${a.buddy?`
                        ...
                        ${d?`<span class="pokemon-name-lite" style="${v(a.buddy.typeColors)}">${a.buddy.name}</span>`:""}
                    `:"N/A"}
                </td>
                <td class="hide-on-mobile">${a.kmWalked} km</td>
                <td class="hide-on-mobile">${a.pokemonCaught.toLocaleString()}</td>
            </tr>
        `).join("");const E=document.getElementById("strongest-pokemon-body"),h=s.strongestPokemon;E.innerHTML=h.map((a,f)=>`
            <tr class="clickable-pokemon-row" data-index="${f}">
                <td>${f+1}</td>
                <td class="pokemon-cell">
                    <img src="${a.sprite}" alt="${a.name}">
                    ${d?`<span class="pokemon-name-lite" style="${v(a.typeColors)}">${a.name}</span>`:""}
                </td>
        ...
        `).join("");const L=document.getElementById("rarest-pokemon-body"),D=s.rarestPokemon;L.innerHTML=D.map((a,f)=>`
        ...
        `).join(""),document.getElementById("rankings-grid").addEventListener("click",a=>{const f=a.target.closest(".clickable-row");if(f){const B=f.dataset.playerId;B&&n(B);return}const I=a.target.closest(".clickable-pokemon-row");if(I){const B=I.dataset.index,S=h[B];S&&b(S);return}const T=a.target.closest(".clickable-rarity-row");if(T){const B=T.dataset.index,S=D[B];S&&y(S)}}),p.classList.add("hidden"),k.classList.remove("hidden");const w=document.querySelector(".info-btn");w&&w.addEventListener("click",a=>{a.stopPropagation(),r()}),window.addEventListener("resize",e),setTimeout(e,100),setTimeout(e,500)}catch(e){console.error("Failed to initialize public dashboard:",e),p.innerHTML="<p>Could not load ranking data. Please try again later.</p>",document.title="Pokemon GO | Dashboard"}l.addEventListener("click",e=>{e.target===l&&(l.classList.add("hidden"),document.body.classList.remove("modal-open"))})});let P=[];async function C(){const p=document.getElementById("events-container"),k="https://raw.githubusercontent.com/bigfoott/ScrapedDuck/data/events.min.json",l={"community-day":"#1660a9","raid-day":"#e74c3c","raid-battles":"#c0392b",event:"#27ae60","raid-hour":"#c0392b",research:"#1abc9c","timed-research":"#1abc9c","limited-research":"#159e83","live-event":"#d63031","pokemon-go-fest":"#153d94","research-breakthrough":"#795548","special-research":"#13a185","global-challenge":"#0a64b5","go-rocket-takeover":"#1e1e1e","team-go-rocket":"#1e1e1e","giovanni-special-research":"#1e272e","safari-zone":"#3d7141","ticketed-event":"#de3e9b","go-battle-league":"#8e44ad","pokemon-spotlight-hour":"#e58e26","bonus-hour":"#40407a",update:"#2980b9","raid-weekend":"#6f1e51","potential-ultra-unlock":"#2c3e50","location-specific":"#284b92",season:"#38ada9","elite-raids":"#a21416","pokemon-go-tour":"#1d3a74","pokestop-showcase":"#ff9f43",default:"#bdc3c7"};function u(c){const $=Date.parse(c)-Date.parse(new Date);if($<=0)return null;const y=Math.floor($/(1e3*60*60*24)),g=Math.floor($/(1e3*60*60)%24),b=Math.floor($/1e3/60%60);return y>0?`${y}d ${g}h`:g>0?`${g}h ${b}m`:`${b}m`}try{const c=await fetch(k);if(!c.ok)throw new Error("Failed to fetch events");P=await c.json();const $=new Date,y=[],g=[];if(P.forEach(n=>{const r=new Date(n.start),o=new Date(n.end);$>=r&&$<=o?y.push(n):$<r&&g.push(n)}),g.sort((n,r)=>new Date(n.start)-new Date(r.start)),y.sort((n,r)=>new Date(n.end)-new Date(r.end)),y.length===0&&g.length===0){p.innerHTML='<p class="no-events">No active or upcoming events found.</p>';return}const b=(n,r)=>{const o=new Date(n.start),e=new Date(n.end),t=r?"Starts in":"Ends in",s=u(r?n.start:n.end),i=l[n.eventType]||l.default;return`
            ...
            `};let v="";y.length>0&&(v+='<h3 class="events-section-header">Ongoing</h3>',v+=y.map(n=>b(n,!1)).join("")),g.length>0&&(v+='<h3 class="events-section-header">Upcoming</h3>',v+=g.map(n=>b(n,!0)).join("")),p.innerHTML=v,p.querySelectorAll(".event-item").forEach(n=>{n.addEventListener("click",()=>{const r=n.getAttribute("data-event-id"),o=P.find(e=>e.eventID===r);o&&j(o,l[o.eventType]||l.default)})});const d=document.getElementById("expand-events-btn");d&&(d.onclick=()=>{R(l)})}catch(c){console.error("Error loading events:",c),p.innerHTML='<p class="error-message">Error loading events.</p>'}}function R(p){const k=document.getElementById("timeline-modal-backdrop"),l=document.getElementById("timeline-scroll-container"),u=document.getElementById("timeline-modal-close-btn");if(!k||!l||!u)return;u.onclick=()=>k.classList.add("hidden"),k.onclick=t=>{t.target===k&&k.classList.add("hidden")};const c=new Date;c.setHours(0,0,0,0);const $=P.filter(t=>new Date(t.end)>=c);let y=new Date(c);$.forEach(t=>{const s=new Date(t.end);s>y&&(y=s)});const g=1440*60*1e3,b=Math.ceil((y-c)/g),v=Math.min(Math.max(14,b),90),d=[];for(let t=0;t<v;t++){const s=new Date(c);s.setDate(c.getDate()+t),d.push(s)}const n=new Date(d[v-1]);n.setHours(23,59,59,999);const r={};$.forEach(t=>{const s=new Date(t.start);new Date(t.end)<c||s>n||(r[t.eventType]||(r[t.eventType]=[]),r[t.eventType].push(t))});let o=`<div class="timeline-grid" style="grid-template-columns: 160px repeat(${v}, 100px);">`;o+='<div class="timeline-header-row">',o+='<div class="timeline-label-header">Event Category</div>';const e=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];d.forEach((t,s)=>{o+=`
            <div class="${`timeline-date-cell${s===0?" today":""}`}">
                <div>${e[t.getDay()]}</div>
                <div>${t.getDate()}</div>
            </div>
        `}),o+="</div>",Object.keys(r).forEach(t=>{const s=r[t],i=t.replace(/-/g," ");s.sort((h,L)=>new Date(h.start)-new Date(L.start));const m=[],x=s.map(h=>{const L=new Date(h.start),D=new Date(h.end);let w=0;L>c&&(w=(L-c)/(1440*60*1e3));let a=v;D<n&&(a=(D-c)/(1440*60*1e3)),w<0&&(w=0),a>v&&(a=v),a<=w&&(a=w+1/24);let f=-1;for(let I=0;I<m.length;I++)if(w>=m[I]-.01){f=I;break}return f===-1?(m.push(a),f=m.length-1):m[f]=a,{event:h,startCol:w,endCol:a,track:f}}),E=16+Math.max(1,m.length)*34;o+='<div class="timeline-row">',o+=`<div class="timeline-row-label" style="height: ${E}px;">${i}</div>`;for(let h=0;h<v;h++)o+=`<div class="${`timeline-grid-cell${h===0?" today-col":""}`}" style="height: ${E}px;"></div>`;o+=`<div class="timeline-bar-wrapper" style="grid-column: 2 / span ${v}; grid-template-columns: repeat(${v}, 100px); height: ${E}px;">`,x.forEach(({event:h,startCol:L,endCol:D,track:w})=>{const a=p[h.eventType]||p.default,f=L*100,I=(D-L)*100-6,T=10+w*34;o+=`
                <div class="timeline-bar" 
                     style="left: ${f+3}px; width: ${I}px; top: ${T}px; background-color: ${a};"
                     title="${h.name}"
                     data-event-id="${h.eventID}">
                    ${h.name}
                </div>
            `}),o+="</div>",o+="</div>"}),o+="</div>",l.innerHTML=o,l.querySelectorAll(".timeline-bar").forEach(t=>{t.addEventListener("click",s=>{s.stopPropagation();const i=t.getAttribute("data-event-id"),m=P.find(x=>x.eventID===i);m&&j(m,p[m.eventType]||p.default)})}),k.classList.remove("hidden")}function j(p,k){const l=document.getElementById("modal-backdrop"),u=document.getElementById("modal-content");if(!l||!u)return;u.style.setProperty("--event-theme",k);const c=new Date,$=new Date(p.start);new Date(p.end);let y="";if(c<$){const d=Date.parse(p.start)-Date.parse(c),n=Math.floor(d/(1e3*60*60*24)),r=Math.floor(d/(1e3*60*60)%24);y=`Starts in: ${n>0?`${n}d ${r}h`:`${r}h`}`}else{const d=Date.parse(p.end)-Date.parse(c),n=Math.floor(d/(1e3*60*60*24)),r=Math.floor(d/(1e3*60*60)%24);y=`Ends in: ${n>0?`${n}d ${r}h`:`${r}h`}`}let g="";const b=p.extraData||{};if(p.eventType==="pokemon-spotlight-hour"&&b.spotlight){const d=b.spotlight;g=`
        ...
        `}else if((p.eventType==="raid-battles"||p.eventType==="raid-hour"||p.eventType==="raid-day")&&b.raidbattles){const d=b.raidbattles,n=d.bosses||[],r=d.shinies||[];g=`
            <div class="event-details-grid">
                ${n.length>0?`
                <div class="event-section-card">
                    <h4>Raid Bosses</h4>
        ...
        `}else if(p.eventType==="community-day"&&b.communityday){const d=b.communityday,n=d.spawns||[],r=d.bonuses||[],o=d.shinies||[],e=d.bonusDisclaimers||[],t=d.specialresearch||[];g=`
            <div class="event-details-grid">
        ...
        `}else{const d=b.generic?.hasSpawns,n=b.generic?.hasFieldResearchTasks;g=`
